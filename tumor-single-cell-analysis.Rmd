...
It is often helpful to determine what proportion of total cells each cell cluster represents. Given a seurat object, the function plot_proportions will plot a bargraph showing the % of total cells for each cluster. It will pull from the active.ident slot.
...
We observe that the majority of cells are TAMs, followed by monocytes and CD8 T cells.
...
We observe that the highest number of transcripts are found in the CD8 T cell cluster and the macrophage cluster. If there is a dead cell cluster, usually it will have the lowest number of transcripts. To get a better grasp on a per-cluster basis, we can use dplyr to summarize the average UMI/nCount_RNA grouped by cluster. We can also summarize other items of interest such as the average nFeature, percent.mito, etc, using the function ```summarize_metadata``` below:
...
Sometimes, I want to zoom in on a subcluster of cells and re-cluster them in order to determine what further heterogeneity exists within that cluster. After doing so, I often want to determine where sub-clusters lie in the original t-SNE plot. You can pull out individual cell barcodes by either using the function ```WhichCells```, or by simplying taking the rownames of the meta.data corresponding to the cells you want.

```{r fig.width=4, fig.heigh=3}
...
TSNEPlot(cd8, label = T)

# select barcodes from cluster 1, using function WhichCells
cd8_cluster1_barcodes <- WhichCells(cd8, idents = 1)
```

```{r fig.width=10, fig.height=5}
# highlight the cells using cells.highlight
ta <- TSNEPlot(b16, label = T, cells.highlight = cd8_cluster1_barcodes) + labs(title = "Using cells.highlight")
...
TSNEPlot(b16, cells.highlight = top_right)

```

After pulling out the barcodes, we observe that this small island is madde up of 13 cells. We can assign these 13 cells a new "ident" within active.ident, and then find the markers that define this small cluster of cells:

```{r}
# add these barcodes to the active.ident as a new cluster called unknown
levels(b16.combined@active.ident) <- c(levels(b16.combined@active.ident), "unknown")
b16.combined@active.ident[top_right] <- "unknown"

# now, this new cluster shows up in our seurat functions
table(b16.combined@active.ident)
TSNEPlot(b16.combined, label = T)
...
```{r}
unknown_markers[order(unknown_markers$avg_logFC, decreasing = T)[1:30],]
```

Based on the FindMarkers result, the 13 cells that make up the unknown cluster look like a non-immune stromal cell type. Perhaps these cells were not captured with other stromal cell types when we first filtered those clusters out, or they could be a small population of doublets.

#### 5. Proportion of positive cells by cluster

Sometimes, it's useful to know how many cells express a given gene at any level (any expression value greater than 0). This finds the proportion of cells within specified clusters that are positive for a given marker. It does so using a simple ifelse statement.

```{r warning=F}
percent_positive <- function(seuratobj, marker, idents = "all"){
...
percent_positive(b16.combined, "Ccr2")
percent_positive(b16.combined, "Ccr2", c("pDC", "CD8", "Monocyte"))
```
...
#### 6. Median split t-SNE plot and proportion plot
...
We can visualize this information by plotting either a side-by-side barplot or a stacked barplot, using the function ```plot_group_proportions``` below:
...
Interestingly, we observe lower amounts of TAMs in the ADAR1 KO samples compared to the control samples, and higher amounts of monocytes and CD8 T cells.

#### 2. Plot heatmap to cluster samples or cell types

Now that we have defined the cell composition by sample, we can perform some correlative analysis between samples in order to determine how similar the cell composition of each sample is to one another. This can be achieved by simply using the ```cor()``` function and plotting a heatmap using ```pheatmap```. We can also visualize which sample has the most of any given cell type by plotting the raw values on the heatmap and scaling by the cell type column.
...
#### 3. Median expression bar graphs split by group
...
Now that we have a cleaned-up and named vector, we can plug it into the fgsea() function along with the hallmark pathway object, which generates a table of results containing the enrichment scores associated with each pathway:
...
We can plot enrichment curves for individual pathways using the built-in ```plotEnrichment``` function, which I've written a small wrapper for below. In these plots, the black ticks on the x-axis indicate a gene in the pathway, and the green curve is a measure of how enriched the genes are for either the ADAR1 KO group (left side) or the Control group (right side). An example of the interferon gamma response pathway is below, along with the IL2 STAT5 signaling pathway, which is highly enriched in the opposite direction towards the control samples.
...
The fsgea_results table above is useful, but it would be nicer to view in a more graphical format. Below, we can construct a "waterfall" plot, which is basically a sideways bar plot of enrichment scores for each of the pathways. The significantly enriched pathways are highlighted in a different color.
...
Using this visualization, we can quickly point out the significantly enriched pathways. It is interesting to note that both interferon response pathways as well as both myc targets pathways are present in the top fraction.
