...
top_markers_immune
# can save the top markers per cluster for analysis records
...
The ```FeatureScatter``` function can be used to assess the correlation between two genes within a cluster. For example, we can examine how CD8 T-cell activation-associated genes such as Gzmb are correlated with exhaustion-associated genes such as Tox.
...
Sometimes, I want to zoom in on a subcluster of cells and re-cluster them in order to determine what further heterogeneity exists within that cluster. After doing so, I often want to determine where sub-clusters lie in the original t-SNE plot. You can pull out individual cell barcodes by either using the function ```WhichCells```, or by simplying taking the rownames of the meta.data corresponding to the cells you want. Below, I further subcluster the CD8 T cell cluster, and select one of the new subcluster's barcodes by using ```WhichCells```:

```{r fig.width=4, fig.heigh=3, message=F, warning=F}
...
After pulling out the barcodes, we observe that this small island is madde up of 13 cells. We can assign these 13 cells a new "ident" within active.ident, and then find the markers that define this small cluster of cells using ```FindMarkers```:
...
Sometimes, it's useful to know how many cells express a given gene at any level (i.e. have any expression value greater than 0). The function ```percent_positive``` finds the proportion of cells within specified clusters that are positive for a given marker. It does so using a simple ```ifelse()``` statement.
...
Similar to the median-split analysis above, this analysis assigns high vs low expression to the cells in the dataset, then calculates the proportion of cells from each group that lie in either the high or low expression category. Below, I split the data based on the PD1 expression.
...
This concludes my analysis! I hope that my workflow and code helps researchers out there analyze their own single cell RNA-sequencing experiments.
